...
const Profile = () => {
  ...
  const userData = {
    ...
    resolvedIssues: 28,
    badges: [
      { name: 'Community Champion', icon: '🏆', description: 'Contributed to 25+ resolved issues', date: 'March 2024' },
      { name: 'First Responder', icon: '⚡', description: 'Quick response to critical issues', date: 'February 2024' },
      { name: 'Problem Solver', icon: '🎯', description: 'Successfully resolved 10 issues', date: 'February 2024' },
      { name: 'Local Hero', icon: '⭐', description: 'Most active user in Koramangala', date: 'March 2024' },
    ],
    recentActivity: [
      { 
        type: 'report', 
    ...
    issues: [
      {
        ...
        upvotes: 18,
        comments: 7,
        description: 'Large amounts of waste being dumped near the park entrance. This needs immediate attention.'
      },
      {
        id: 2,
        ...
        status: 'verified',
        location: '80 Feet Road and 100 Feet Road Junction, Koramangala',
        reportedDate: 'March 25, 2024',
        updatedDate: 'April 8, 2024',
        upvotes: 37,
        comments: 12,
        description: 'Major road damage causing traffic congestion and safety hazards.'
      },
      {
        id: 3,
        ...
        upvotes: 15,
        comments: 5,
        description: 'Water leakage in public restroom causing wastage and hygiene issues.'
      }
    ]
  };
  ...
  const getCategoryColor = (category) => {
    switch (category.toLowerCase()) {
      case 'environmental': return 'bg-green-100 text-green-600';
      case 'infrastructure': return 'bg-orange-100 text-orange-600';
      case 'plumbing': return 'bg-blue-100 text-blue-600';
      case 'electrical': return 'bg-yellow-100 text-yellow-600';
      case 'sanitation': return 'bg-purple-100 text-purple-600';
                ...
                {[
                  ...
                  >
                    <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center mr-3">
                      <InfoIcon category={cat} className="text-blue-600" size={18} />
                    </div>
        ...
        <div className="my-8 bg-white rounded-2xl w-full max-w-3xl shadow-2xl transform transition-all scale-100 animate-slideIn">
          <div className="px-8 py-6">
            <div className="flex justify-between items-start mb-6">
              <div>
                <span className={`text-xs font-medium px-2 py-1 rounded-md ${getCategoryColor(selectedIssue.category)}`}>
                  {selectedIssue.category}
                </span>
                <h2 className="text-xl font-bold text-gray-800 mt-2">{selectedIssue.title}</h2>
                <p className="text-sm text-gray-500">Issue #{selectedIssue.id}</p>
              </div>
              <div className="flex items-center space-x-3">
                <span className={`text-xs px-2 py-1 rounded-full ${getStatusColor(selectedIssue.status)} text-white`}>
                  {getStatusText(selectedIssue.status)}
                </span>
                <button onClick={() => setShowViewDetailsModal(false)} className="text-gray-500 hover:text-gray-700">
                  <XIcon size={24} />
                </button>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                <h3 className="font-medium text-gray-800 mb-2">Timeline</h3>
                <div className="space-y-3">
                  <div className="flex items-start">
                    <div className="w-2 h-2 mt-2 rounded-full bg-blue-600"></div>
                    <div className="ml-4">
                      <div className="text-sm font-medium text-gray-800">Issue Reported</div>
                      <div className="text-xs text-gray-500">{selectedIssue.reportedDate}</div>
                    </div>
                  </div>
                  <div className="flex items-start">
                    ...
                    <div className="ml-4">
                      ...
                      <div className="text-xs text-gray-500">{selectedIssue.updatedDate}</div>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <div className="min-h-screen bg-gray-100">
          ...
          <div className="relative">
            ...
            {showProfileDropdown && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2 z-10 transition-all duration-200">
                <a href="#" className="block px-4 py-2 text-gray-700 hover:bg-gray-100 transition-colors">Account Settings</a>
                <a href="#" className="block px-4 py-2 text-gray-700 hover:bg-gray-100 transition-colors">Privacy Settings</a>
                <a href="#" className="block px-4 py-2 text-gray-700 hover:bg-gray-100 transition-colors">Notification Preferences</a>
                <div className="border-t border-gray-200 mt-1 pt-1">
                  <a href="#" className="block px-4 py-2 text-red-600 hover:bg-gray-100 transition-colors">Sign Out</a>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-2 py-6">
              ...
              <div>
                ...
                <div className="flex items-center mt-3 space-x-6">
                  <div className="text-sm">
                    ...
                    <span className="ml-1 font-medium">{userData.joinedDate}</span>
                  </div>
                  <div className="text-sm">
                    <span className="text-gray-500">Total Reports</span>
                    <span className="ml-1 font-medium">{userData.totalReports}</span>
                  </div>
                  <div className="text-sm">
                    <span className="text-gray-500">Resolved</span>
                    <span className="ml-1 font-medium">{userData.resolvedIssues}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="col-span-3 grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              <div className="bg-white rounded-lg p-4 shadow-sm hover:shadow-md transition-all">
                ...
                <div className="text-2xl font-bold text-blue-600">{userData.issues.length}</div>
              ...
              <div className="bg-white rounded-lg p-4 shadow-sm hover:shadow-md transition-all">
                ...
                <div className="text-2xl font-bold text-amber-600">
                  {userData.issues.filter(i => i.status === 'in-progress').length}
                </div>
              ...
              <div className="bg-white rounded-lg p-4 shadow-sm hover:shadow-md transition-all">
                ...
                <div className="text-2xl font-bold text-green-600">
                  {userData.issues.filter(i => i.status === 'resolved').length}
                </div>
              ...
              <div className="bg-white rounded-lg p-4 shadow-sm hover:shadow-md transition-all">
                ...
                <div className="text-2xl font-bold text-purple-600">
                  {userData.issues.reduce((acc, i) => acc + i.upvotes, 0)}
                </div>
                <div className="text-xs text-gray-400 mt-1">Community support</div>
              </div>
            </div>
            ...
            <div className="col-span-3 md:col-span-2">
              <div className="bg-white rounded-lg shadow-sm p-4">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-bold text-gray-800">Recent Issues</h2>
                  <button onClick={() => setActiveTab('issues')} className="text-sm text-blue-600 hover:text-blue-800">View All</button>
                </div>
                <div className="space-y-4">
                  {userData.issues.slice(0, 2).map((issue, index) => (
                    <div key={index} className="border border-gray-100 rounded-lg p-3 hover:shadow-md transition-all">
                      <div className="flex justify-between items-start">
                        <div>
                          <span className="text-xs font-medium px-2 py-1 rounded-md bg-blue-100 text-blue-600">
                            {issue.category}
                          </span>
                          <h3 className="font-medium text-gray-800 mt-2">{issue.title}</h3>
                          <p className="text-sm text-gray-600 mt-1">{issue.description}</p>
                        </div>
                        <span className={`text-xs px-2 py-1 rounded-full ${getStatusColor(issue.status)} text-white`}>
            ...
            <div className="col-span-3 md:col-span-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white rounded-lg shadow-sm p-6 transform hover:shadow-md transition-all duration-300">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-bold text-gray-800 flex items-center">
                      <TrophyIcon size={18} className="mr-2 text-amber-500" />
                      Recent Badges
                    </h2>
                    <span className="text-sm text-blue-600 cursor-pointer hover:text-blue-800 hover:underline">View All</span>
                  </div>
                  <div className="space-y-4">
                    {userData.badges.slice(0, 2).map((badge, index) => (
                      <div key={index} className="flex items-center p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg transform hover:scale-102 hover:shadow-md transition-all duration-300">
                        <div className="text-3xl mr-4">{badge.icon}</div>
                        <div>
                          <div className="font-medium text-gray-800">{badge.name}</div>
                          <div className="text-sm text-gray-600">{badge.description}</div>
                          <div className="text-xs text-blue-600 mt-1">Earned {badge.date}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="bg-white rounded-lg shadow-sm p-6 transform hover:shadow-md transition-all duration-300">
                  ...
                  <div className="space-y-4">
                    {userData.popularComments.map((comment, index) => (
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            <div className="text-xs text-gray-500">
                              <span className="font-medium text-blue-600">{comment.issue}</span>
                              <span className="mx-2">•</span>
                              <span>{comment.date}</span>
                            </div>
                            {comment.isHot && (
                              <div className="flex items-center text-xs text-orange-500">
                                <FireExtinguisher size={12} className="mr-1" />
                                <span>Hot</span>
                              </div>
                            )}
                          </div>
                          <div className="flex items-center space-x-4">
                            ...
                            <button 
                              ...
                              className={`flex items-center space-x-1 group transition-all duration-300 ${
                                upvotedComments[comment.id] || comment.isUpvoted ? 'text-blue-600' : 'text-gray-500 hover:text-blue-600'
                              }`}
                            >
                              <div className="relative">
                                <ThumbsUpIcon 
                                  ...
                                  className={`transform transition-all duration-300 ${
                                    upvotedComments[comment.id] || comment.isUpvoted ? 'scale-125' : 'group-hover:scale-110'
                                  }`}
                                />
                                {!upvotedComments[comment.id] && !comment.isUpvoted && (
                              ...
                              <span className={`text-xs font-medium transition-all duration-300 ${
                                upvotedComments[comment.id] || comment.isUpvoted ? 'text-blue-600' : ''
                              }`}>
                                {comment.upvotes}
                              </span>
                            </button>
        ...
        {activeTab === 'issues' && (
          <div className="bg-white rounded-lg shadow-sm p-4">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-gray-800">All Issues</h2>
              <div className="flex items-center space-x-3">
                <button 
                  onClick={() => setShowPostIssueModal(true)}
                ...
                <div key={index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-all">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <span className={`text-xs font-medium px-2 py-1 rounded-md ${getCategoryColor(issue.category)}`}>
                        {issue.category}
                      </span>
                      <h3 className="text-lg font-medium text-gray-800 mt-2">{issue.title}</h3>
                      <p className="text-sm text-gray-600 mt-1">{issue.description}</p>
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full ${getStatusColor(issue.status)} text-white ml-4`}>
                      {getStatusText(issue.status)}
                    </span>
                  </div>
        ...
        {activeTab === 'badges' && (
          <div className="bg-white rounded-lg shadow-sm p-6 transform hover:shadow-md transition-all duration-300">
            ...
            <div className="mt-8 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg">
              <h3 className="text-md font-bold text-gray-800 mb-4">Upcoming Badges</h3>
              <div className="space-y-3">
                <div className="flex items-center">
                  <div className="text-2xl mr-4">🏅</div>
                  <div>
                    <div className="font-medium text-gray-800">Quality Contributor</div>
                    <div className="text-sm text-gray-600">Receive 50+ upvotes on your comments</div>
                    <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                      <div className="bg-blue-600 h-2 rounded-full" style={{ width: '70%' }}></div>
                    </div>
                    <div className="text-xs text-blue-600 mt-1">Progress: 35/50 upvotes</div>
                  </div>
                </div>
                <div className="flex items-center">
                  <div className="text-2xl mr-4">🔍</div>
                  <div>
                    <div className="font-medium text-gray-800">Eagle Eye</div>
                    <div className="text-sm text-gray-600">Report 10 issues that get verified by authorities</div>
                    <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                      <div className="bg-blue-600 h-2 rounded-full" style={{ width: '40%' }}></div>
                    </div>
                    <div className="text-xs text-blue-600 mt-1">Progress: 4/10 verified reports</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'activity' && (
          <div className="bg-white rounded-lg shadow-sm p-6 transform hover:shadow-md transition-all duration-300">
            <h2 className="text-lg font-bold text-gray-800 mb-6 flex items-center">
              <HistoryIcon size={18} className="mr-2 text-blue-500" />
              Activity History
            </h2>
            ...
            <div className="flex items-center space-x-4 mb-6">
              <div className="px-4 py-2 bg-blue-50 rounded-lg text-center flex-1">
                ...
                <div className="text-xl font-bold text-blue-600">{userData.totalReports}</div>
              </div>
              <div className="px-4 py-2 bg-green-50 rounded-lg text-center flex-1">
                <div className="text-sm text-gray-600">Resolved Issues</div>
                <div className="text-xl font-bold text-green-600">{userData.resolvedIssues}</div>
              </div>
              <div className="px-4 py-2 bg-amber-50 rounded-lg text-center flex-1">
                <div className="text-sm text-gray-600">Success Rate</div>
                <div className="text-xl font-bold text-amber-600">{Math.round((userData.resolvedIssues / userData.totalReports) * 100)}%</div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {userData.recentActivity.map((activity, index) => (
                <div key={index} className="bg-gray-50 rounded-lg p-4 hover:bg-gray-100 transition-all duration-300">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start">
                      <div className={`w-3 h-3 rounded-full mt-1.5 mr-4 ${getStatusColor(activity.status)}`} />
                      <div>
                        ...
                        <div className="flex items-center space-x-4 mt-2">
                          <div className="flex items-center space-x-1 text-gray-500">
                            ...
                            <span className="text-xs">{activity.upvotes} upvotes</span>
                          </div>
                          <div className="flex items-center space-x-1 text-gray-500">
                            <MessageCircleIcon size={14} />
                            <span className="text-xs">{activity.comments} comments</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <span className={`text-xs px-2 py-1 rounded-full ${getStatusColor(activity.status)} text-white`}>
            ...
            <div className="grid grid-cols-1 gap-4">
              {userData.trendingIssues.map((issue, index) => (
                <div key={index} className="p-4 border border-gray-200 rounded-lg hover:shadow-md transition-all duration-300">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="text-lg font-medium text-gray-800">{issue.title}</div>
